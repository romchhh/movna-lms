...
export interface TeacherSalariesPanelProps {
  embedded?: boolean
  /** Admin: Optimate teacher id for salary lookup */
  teacherId?: string
}

export function TeacherSalariesPanel({ embedded = false, teacherId }: TeacherSalariesPanelProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = teacherId
        ? await adminOptimateApi.teacherTransactions(
          teacherId,
          page,
          25,
          period.from,
        ...
        : await teacherOptimateApi.transactions(
          page,
          25,
          period.from,
          period.to,
  ...
  }, [page, period.from, period.to, canLoad, teacherId])
  ...
  useEffect(() => {
    setPage(1)
  }, [preset, customFrom, customTo, teacherId])
